...
bool static
CheckPubKeyEncoding(const valtype &vchPubKey, unsigned int flags, const SigVersion &sigversion, ScriptError *serror) {
...
PrecomputedTransactionData::PrecomputedTransactionData(const T &txTo) {
    hashPrevouts = SHA256Uint256(GetPrevoutSHA256(txTo));
    hashSequence = SHA256Uint256(GetSequenceSHA256(txTo));
...
template uint256
SignatureHash<CMutableTransaction>(const CScript &scriptCode, const CMutableTransaction &txTo, unsigned int nIn,
                                   int nHashType, const CAmount &amount, SigVersion sigversion,
                                   const PrecomputedTransactionData *cache);

template uint256
SignatureHash<CTransaction>(const CScript &scriptCode, const CTransaction &txTo, unsigned int nIn, int nHashType,
                            const CAmount &amount, SigVersion sigversion, const PrecomputedTransactionData *cache);

template<class T>
bool
GenericTransactionSignatureChecker<T>::VerifySignature(const std::vector<unsigned char> &vchSig, const CPubKey &pubkey,
...
// explicit instantiation
template
class GenericTransactionSignatureChecker<CTransaction>;

template
class GenericTransactionSignatureChecker<CMutableTransaction>;
