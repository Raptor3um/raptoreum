...
CTransactionBuilderOutput::CTransactionBuilderOutput(CTransactionBuilder *pTxBuilderIn,
                                                     std::shared_ptr <CWallet> pwalletIn, CAmount nAmountIn) :
...
CTransactionBuilder::CTransactionBuilder(std::shared_ptr <CWallet> pwalletIn, const CompactTallyItem &tallyItemIn) :
    ...
    coinControl.m_feerate = std::max(::feeEstimator.estimateSmartFee((int) pwallet->m_confirm_target, nullptr, true),
                                     pwallet->m_pay_tx_fee);
    ...
    nBytesBase = ::GetSerializeSize(CTransaction(dummyTx), SER_NETWORK, PROTOCOL_VERSION);
...
CAmount
CTransactionBuilder::GetAmountLeft(const CAmount nAmountInitial, const CAmount nAmountUsed, const CAmount nFee) {
...
bool CTransactionBuilder::Commit(std::string &strResult) {
    ...
    if (nChangePosRet != -1 && fDust) {
        strResult = strprintf("Unexpected change output %s at position %d", tx->vout[nChangePosRet].ToString(),
                              nChangePosRet);
        return false;
    }
    ...
    if (nFeeRet != nFeeCalc) {
        strResult = strprintf(
                "Fee validation failed -> nFeeRet: %d, nFeeCalc: %d, nFeeAdditional: %d, nBytesAdditional: %d, %s",
                nFeeRet, nFeeCalc, nFeeAdditional, nBytesAdditional, ToString());
        return false;
    }
...
std::string CTransactionBuilder::ToString() const {
    return strprintf(
            "CTransactionBuilder(Amount initial: %d, Amount left: %d, Bytes base: %d, Bytes output: %d, Bytes total: %d, Amount used: %d, Outputs: %d, Fee rate: %d, Discard fee rate: %d, Fee: %d)",
            GetAmountInitial(),
            GetAmountLeft(),
            nBytesBase,
            nBytesOutput,
