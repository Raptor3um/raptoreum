...
std::ostream& operator<<(std::ostream& out, EUpdateState state)
{
   switch (state)
   {
      case EUpdateState::Defined  : out << "Defined";   break;
      case EUpdateState::Voting   : out << "Voting";    break;
      case EUpdateState::LockedIn : out << "Locked In"; break;
      case EUpdateState::Active   : out << "Active";    break;
      case EUpdateState::Failed   : out << "Failed";    break;
   }
   return out;
}
      ...
      for (size_t i = 1; i < block.vtx.size(); i++)
      {
         ...
         if (tx->nType == TRANSACTION_QUORUM_COMMITMENT)
         {
            ...
            if (!GetTxPayload(*tx, qc)) {
               LogPrint(BCLog::UPDATES, "Updates: NodeRoundVoting, Height: %d, Tx: %d, Quorum Commitment - GetTxPayload failed\n", curIndex->nHeight, i);
            }
...
VoteResult MinerUpdateVoting::GetVote(const CBlockIndex* blockIndex, const Update &update)
{
   ...
   {
      const CBlockIndex* roundBlockIndex = blockIndex->GetAncestor(blockIndex->nHeight - lookback * update.RoundSize());
