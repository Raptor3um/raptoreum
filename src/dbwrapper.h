...
public:
     ...
     */
    explicit CDBBatch(const CDBWrapper &_parent) : parent(_parent), ssKey(SER_DISK, CLIENT_VERSION),
...
protected:
    ...
    ssize_t memoryUsage{
            0}; // signed, just in case we made an error in the calculations so that we don't get an overflow
