...
private:
    ...
    std::map<std::pair<Consensus::LLMQType, uint256>, uint256> mineableCommitmentsByQuorum GUARDED_BY(mineableCommitmentsCs);
    std::map<uint256, CFinalCommitment> mineableCommitments GUARDED_BY(mineableCommitmentsCs);

    mutable std::map<Consensus::LLMQType, unordered_lru_cache<uint256, bool, StaticSaltedHasher>> mapHasMinedCommitmentCache GUARDED_BY(mineableCommitmentsCs);
