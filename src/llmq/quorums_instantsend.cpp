...
{
    LOCK(cs_db);
    return GetInstantSendLockByHashInternal(islockHash) != nullptr || db->Exists(std::make_tuple(std::string(DB_ARCHIVED_BY_HASH), islockHash));
}
...
bool CInstantSendManager::CheckCanLock(const CTransaction& tx, bool printDebug, const Consensus::Params& params) const
{
    ...
    return ranges::all_of(tx.vin, [&](const auto& in){ return CheckCanLock(in.prevout, printDebug, tx.GetHash(), params); });
}
...
void CInstantSendManager::ProcessMessageInstantSendLock(const CNode* pfrom, const llmq::CInstantSendLockPtr& islock)
{
    ...
    if (WITH_LOCK(cs_pendingLocks, return pendingInstantSendLocks.count(hash) || pendingNoTxInstantSendLocks.count(hash)) || db.KnownInstantSendLock(hash)) {
        return;
    }
...
{
    if (!fUpgradedDB) {
        if (WITH_LOCK(cs_llmq_vbc, return VersionBitsState(pindexNew, Params().GetConsensus(), Consensus::DEPLOYMENT_V17, llmq_versionbitscache) == ThresholdState::ACTIVE)) {
            db.Upgrade();
            fUpgradedDB = true;
        }
    }
...
bool CInstantSendManager::AlreadyHave(const CInv& inv) const
{
    ...
    return WITH_LOCK(cs_pendingLocks, return pendingInstantSendLocks.count(inv.hash) != 0 || pendingNoTxInstantSendLocks.count(inv.hash) != 0) || db.KnownInstantSendLock(inv.hash);
}
...
bool CInstantSendManager::IsWaitingForTx(const uint256& txHash) const
{
    ...
    while (it != pendingNoTxInstantSendLocks.end()) {
        if (it->second.second->txid == txHash) {
            LogPrint(BCLog::INSTANTSEND, "CInstantSendManager::%s -- txid=%s, islock=%s\n", __func__, txHash.ToString(), it->first.ToString());
            return true;
        }
        ++it;
    }
