...
struct UISignals {
    ...
    boost::signals2::signal <CClientUIInterface::NotifySmartnodeListChangedSig> NotifySmartnodeListChanged;
...
ADD_SIGNALS_IMPL_WRAPPER(NotifySmartnodeListChanged);

ADD_SIGNALS_IMPL_WRAPPER(NotifyAdditionalDataSyncProgressChanged);

ADD_SIGNALS_IMPL_WRAPPER(BannedListChanged);

bool CClientUIInterface::ThreadSafeMessageBox(const std::string &message, const std::string &caption,
                                              unsigned int style) {
...
bool CClientUIInterface::ThreadSafeQuestion(const std::string &message, const std::string &non_interactive_message,
    ...
    return g_ui_signals.ThreadSafeQuestion(message, non_interactive_message, caption, style);
}

void CClientUIInterface::InitMessage(const std::string &message) { return g_ui_signals.InitMessage(message); }

void CClientUIInterface::NotifyNumConnectionsChanged(
        int newNumConnections) { return g_ui_signals.NotifyNumConnectionsChanged(newNumConnections); }
...
void CClientUIInterface::NotifyChainLock(const std::string &bestChainLockHash, int bestChainLockHeight) {
    return g_ui_signals.NotifyChainLock(bestChainLockHash, bestChainLockHeight);
}

void CClientUIInterface::NotifyHeaderTip(bool b, const CBlockIndex *i) { return g_ui_signals.NotifyHeaderTip(b, i); }

void CClientUIInterface::NotifySmartnodeListChanged(
        const CDeterministicMNList &list) { return g_ui_signals.NotifySmartnodeListChanged(list); }

void CClientUIInterface::NotifyAdditionalDataSyncProgressChanged(
        double nSyncProgress) { return g_ui_signals.NotifyAdditionalDataSyncProgressChanged(nSyncProgress); }
