...
namespace {
    void
    checkRange(TrafficGraphData &trafficGraphData, int size, TrafficGraphData::GraphRange toRange, int multiplier) {
...
namespace {
    ...
    void testRangeSwitch(TrafficGraphData::GraphRange baseRange, TrafficGraphData::GraphRange toRange, int size) {
        ...
        for (int i = 1; i <= size; i++) {
            int in = QRandomGenerator::global()->bounded(1000);
            int out = QRandomGenerator::global()->bounded(1000);
...
void TrafficGraphDataTests::averageBandwidthTest() {
    ...
    float rate = step * TrafficGraphData::DESIRED_DATA_SAMPLES /
                 (TrafficGraphData::RangeMinutes[TrafficGraphData::Range_5m] * 60.0f) / 1024.0f; // in KB/s
    ...
    QCOMPARE(trafficGraphData.getCurrentRangeQueueWithAverageBandwidth().size(),
             TrafficGraphData::DESIRED_DATA_SAMPLES);
...
void TrafficGraphDataTests::averageBandwidthEvery2EmptyTest() {
    ...
    int step = 123; // in bytes
    float rate = step * TrafficGraphData::DESIRED_DATA_SAMPLES /
                 (TrafficGraphData::RangeMinutes[TrafficGraphData::Range_5m] * 60.0f) / 1024.0f / 2.0f; // in KB/s
    quint64 totalBytesRecv = 0;
    quint64 totalBytesSent = 0;
    for (int i = 1; i <= TrafficGraphData::DESIRED_DATA_SAMPLES; i++) {
        if (i % 2 == 0) {
