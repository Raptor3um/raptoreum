...
const int TrafficGraphData::DESIRED_DATA_SAMPLES =
        TrafficGraphData::RangeMinutes[TrafficGraphData::Range_5m] * 60; // i.e. one data sample per second for Range_5m
...
TrafficGraphData::SampleQueue TrafficGraphData::sumEach3Samples(const SampleQueue &rangeQueue, GraphRange range) {
    ...
    while (lastUnusedSample - 2 >= 0) {
        result.push_front(rangeQueue.at(lastUnusedSample) + rangeQueue.at(lastUnusedSample - 1) +
                          rangeQueue.at(lastUnusedSample - 2));
        lastUnusedSample -= 3;
    }
    return result;
}
