...
public:
    ...
    noexcept : m_data(nullptr), m_size(
    0) {}
    ...
    constexpr Span(T *begin, std::size_t size)

    noexcept : m_data(begin), m_size(size) {}
     ...
     */
    template<typename T, typename std::enable_if<std::is_convertible<T (*)[], C (*)[]>::value, int>::type = 0>
    CONSTEXPR_IF_NOT_DEBUG Span(T *begin, T *end)

    noexcept : m_data(begin), m_size(end
    - begin)
    {
    ...
    constexpr Span(const Span<O> &other)

    noexcept : m_data(other.m_data), m_size(other
    .m_size) {}
    ...
    template<int N>
    constexpr Span(C (&a)[N])

    noexcept : m_data(a), m_size(N) {}
     ...
     */
    template<typename V, typename std::enable_if<(std::is_const<C>::value || std::is_lvalue_reference<V>::value) &&
                                                 std::is_convertible<typename std::remove_pointer<decltype(
                                                         std::declval<V &>().data())>::type (*)[], C (*)[]>::value &&
                                                 std::is_convertible<decltype(
                                                         std::declval<V &>().size()), std::size_t>::value, int>::type = 0>
    constexpr Span(V &&v)

    noexcept : m_data(v.data()), m_size(v
    .
    ...
    *
    ...
    noexcept { return m_data + m_size; }
    CONSTEXPR_IF_NOT_DEBUG C
    &
    ...
    friend constexpr bool operator==(const Span &a, const Span &b)

    noexcept { return a.size() == b.size() && std::equal(a.begin(), a.end(), b.begin()); }
    ...
    noexcept { return std::lexicographical_compare(a.begin(), a.end(), b.begin(), b.end()); }
...
/** MakeSpan for arrays: */
template<typename A, int N> Span<A> constexpr
MakeSpan(A(&a)
[N]) {
return
Span<A>(a, N);
...
-> typename std::enable_if<!std::is_lvalue_reference<V>::value, Span<const typename std::remove_pointer<decltype(
        v.data())>::type>>::type {
return
std::forward<V>(v);
}
...
UCharSpanCast (MakeSpan(std::forward<V>(v)));
