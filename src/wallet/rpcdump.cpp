    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "as change, and not show up in many RPCs.\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Raptoreum address (or hex-encoded script)"},
                    {"label", RPCArg::Type::STR, /* opt */ true, /* default_val */ "\"\"", "An optional label"},
                    {"rescan", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ "true", "Rescan the wallet for transactions"},
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Raptoreum address for the private key"},
                },
                RPCResult{
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The filename with path (either absolute or relative to raptoreumd)"},
                },
                RPCResult{
            "{                           (json object)\n"
    ...
    std::string strWarning = strprintf(_("%s file contains all private keys from this wallet. Do not share it with anyone!"), request.params[0].get_str());
...
static UniValue ProcessImport(CWallet * const pwallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    try {
        ...
        std::string& label = data.exists("label") ? data["label"].get_str() : "";
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    if (mainRequest.fHelp || mainRequest.params.size() < 1 || mainRequest.params.size() > 2)
        ...
        "                                                              creation time of all keys being imported by the importmulti call will be scanned.",
                                    ...
                                    {"range", RPCArg::Type::RANGE, /* opt */ true, /* default_val */ "", "If a ranged descriptor is used, this specifies the start and end of the range (in the form [begin,end] to import"},
