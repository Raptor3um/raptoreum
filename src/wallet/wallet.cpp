...
static void ReleaseWallet(CWallet* wallet)
{
    ...
    // so that it is in sync with the current chainstate.
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const uint256& block_hash, int posInBlock, bool fUpdate, bool rescanningOldBlock)
{
...
void CWallet::SyncTransaction(const CTransactionRef& ptx, const uint256& block_hash, int posInBlock, bool update_tx, bool rescanningOldBlock) {
    if (!AddToWalletIfInvolvingMe(ptx, block_hash, posInBlock, update_tx, rescanningOldBlock))
 ...
 *            of the chain tip
 *
 * @return ScanResult indicating success or failure of the scan.
 * SUCCESS if scan was successful.
 * FAILURE if a complete rescan was not possible due to pruning or corruption.
 * USER_ABORT if the rescan was aborted before it could complete.
 *
        ...
        while (block_height && !fAbortRescan && !ShutdownRequested()) {
            ...
            if (chain().findBlock(block_hash, &block) && !block.IsNull()) {
                ...
                for (size_t posInBlock = 0; posInBlock < block.vtx.size(); ++posInBlock) {
                    SyncTransaction(block.vtx[posInBlock], block_hash, posInBlock, fUpdate, true);
                }
...
CAmount CWalletTx::GetCredit(interfaces::Chain::Lock& locked_chain, const isminefilter& filter, isminefilter* mineTypes) const
{
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, const CoinEligibilityFilter& eligibility_filter, std::vector<OutputGroup> groups, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CoinSelectionParams& coin_selection_params, bool& bnb_used, CoinType nCoinType) const
{
    setCoinsRet.clear();
    nValueRet = 0;
        ...
        LOCK2(mempool.cs, cs_wallet);
        {
            ...
            if (!boost::get<CNoDestination>(&coin_control.destChange)) {
                ...
                if (IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
                  strFailReason = _("Can not generate a change-address key. Private keys are disabled for this wallet.");
                  return false;
                }
                CPubKey vchPubKey;
                bool ret;
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    if (fFirstRun)
    {
      ...
      // Secure against disabling keys after creation
      InitError(strprintf(_("Error loading %s: Private keys can only be disabled during creation"), walletFile));
      return NULL;
    } else if (walletInstance->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
      LOCK(walletInstance->cs_KeyStore);
      if (!walletInstance->mapKeys.empty() || !walletInstance->mapCryptedKeys.empty()) {
        InitWarning(strprintf(_("Warning: Private keys detected in wallet {%s} with disabled keys"), walletFile));
      }
    }
    ...
    if (gArgs.IsArgSet("-fallbackfee")) {
        CAmount nFeePerK = 0;
        if (!ParseMoney(gArgs.GetArg("-falbackfee", ""), nFeePerK)) {
            InitError(strprintf(_("Invalid amount for -fallbackfee=<amount>: '%s'"), gArgs.GetArg("-fallbackfee", "")));
            return nullptr;
        }
        if (nFeePerK > HIGH_TX_FEE_PER_KB) {
            InitWarning(AmountHighWarn("-fallbackfee") + " " + _("This is the transaction fee you may pay when fee estimates are not available."));
        }
    ...
    if (gArgs.IsArgSet("-discardfee")) {
        ...
        if (nFeePerK > HIGH_TX_FEE_PER_KB) {
            InitWarning(AmountHighWarn("-discardfee") + " " + _("This is the transaction fee you may discard if change is smaller than dust at this level"));
        }
    ...
    if (gArgs.IsArgSet("-paytxfee")) {
        ...
        if (nFeePerK > HIGH_TX_FEE_PER_KB) {
            InitWarning(AmountHighWarn("-paytxfee") + " " + _("This is the transaction fee you will pay if you send a transaction."));
        }
        walletInstance->m_pay_tx_fee = CFeeRate(nFeePerK, 1000);
        if (walletInstance->m_pay_tx_fee < ::minRelayTxFee) {
            InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must bee at least %s)"), gArgs.GetArg("-paytxfee", ""), ::minRelayTxFee.ToString()));
            return nullptr;
        }
    }
