...
### Downgrade of smartnodes to < 0.16

Starting with this release, smartnodes will verify protocol versions of other
smartnodes. This will cause PoSe punishment/banning of out of date smartnodes,
so it is not recommended to downgrade smartnodes.
...
formation of smartnodes and was voted in by the network. The resulting allocation
will split all non-proposal block rewards 40% toward miners and 60% toward
smartnodes in the end-state once the transition period is complete.
...
Increased number of smartnode connections
------------------------------------------
...
connection count for smartnodes. These intra-quorum connections are less resource
demanding than normal p2p connections as they only exchange LLMQ/smartnode related
...
one of a pair of two smartnodes need to initiate the connection while the other one can
wait for an incoming connection. Probing is done in the case where a smartnode doesn't
...
Smartnode Minimum Protocol Version Checks
------------------------------------------
Members of LLMQs will now also check all other members for minimum protocol versions
while in DKG. If a smartnode determines that another LLMQ member has a protocol
...
in the final stages of the DKG. This causes the bad smartnode to get PoSe punished
...
Since these optimizations are exclusive to linux, it is possible that smartnodes hosted
...
- `getpeerinfo` has new field `smartnode` to indicate whether connection was
  due to smartnode connection attempt
...
- `quorum dkgstatus` shows `quorumConnections` for each LLMQ with detailed
  information about each participating smartnode
...
- [`2bb94c73eb`](https://github.com/dashpay/dash/commit/2bb94c73eb) smartnode|net|rpc: Improve smartnode sync process (#3690)
- [`aa81a06264`](https://github.com/dashpay/dash/commit/aa81a06264) smartnode|rpc: Remove unused code (#3689)
...
- [`139f589b6f`](https://github.com/dashpay/dash/commit/139f589b6f) Don'd send SENDXXX messages to fSmartnode connections (#3537)
...
- [`96faa8155e`](https://github.com/dashpay/dash/commit/96faa8155e) Don't disconnect Smartnode probes for a few seconds (#3449)
...
- [`3d24290bcb`](https://github.com/dashpay/dash/commit/3d24290bcb) Take all nodes into account in check_sigs instead of just just smartnodes (#3437)
