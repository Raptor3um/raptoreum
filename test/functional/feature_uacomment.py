...
class UacommentTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.stop_node(0)
        expected = r"ErrorTotal length of network version string \([0-9]+\) exceeds maximum length \(256\). Reduce the number or size of uacomments."
        self.nodes[0].assert_start_raises_init_error(["-uacomment=" + 'a' * 256], expected)

        self.log.info("test -uacomment unsafe characters")
        for unsafe_char in ['/', ':', '(', ')', '₿', '🏃']:
            expected = r"ErrorUser Agent comment \(" + re.escape(unsafe_char) + r"\) contains unsafe characters."
