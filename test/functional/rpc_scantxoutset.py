...
class ScantxoutsetTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # The wallet is deleted a few lines below, so anything still unmined here
        # is lost and the scans further down come up short by an amount that says
        # nothing about why. This test has been seen to fail that way about one
        # run in eight; check the mempool drained so the next one explains itself.
        self.nodes[0].generate(1)
        left = self.nodes[0].getrawmempool()
        assert_equal(left, [])
