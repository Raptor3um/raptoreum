...
Ported from Dash v18.0.0. Two adaptations: the shared cache mines to an
address no wallet holds a key for, so this generates its own chain; and
there is no testmempoolaccept here, so the txid comes from
...
class P2PBlocksOnly(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = self.nodes[0].decoderawtransaction(sigtx)['txid']
