...
Ported from Dash v18.0.0. Raptoreum's test_runner.py has always listed this
script and its --usecli variant, but the file itself was never added: Dash only
introduced it after the 0.17 branch Raptoreum forked from, and Dash's own runner
gained the entries in the same release as the file. Raptoreum picked up the
runner lines from a newer upstream without the test.
...
class CreateWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # end TODO
        # Upstream warns on an empty passphrase; this tree refuses it outright.
        assert_raises_rpc_error(-16, 'Cannot encrypt a wallet with a blank password',
                                self.nodes[0].createwallet,
                                wallet_name='w7', disable_private_keys=False,
                                blank=False, passphrase='')

        # A wallet created without a passphrase at all is unencrypted, and
        # walletpassphrase has nothing to unlock.
        self.nodes[0].createwallet(wallet_name='w7', disable_private_keys=False, blank=False)
