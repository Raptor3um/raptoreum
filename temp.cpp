...
void CDeterministicMNList::printList(const char* method) const {
    LogPrintf("%s::list at %d --------------------------------------------------------\n", method, nHeight);
    ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
        CTxDestination dest;
        if(ExtractDestination(dmn->pdmnState->scriptPayout, dest)) {
            LogPrintf("GetMNPayee::%s\n",
                      CBitcoinAddress(dest).ToString());
        }
    });
    LogPrintf("%s:: end list at %d -------------------------------------------------------- \n", method, nHeight);
}
...
//    }
    ...
    int nHeight = chainActive.Tip() == nullptr ? 0 : chainActive.Tip()->nHeight;
    LogPrintf("GetMNPayee::list at %d --------------------------------------------------------\n", nHeight);
    ...
    CTxDestination dest;
    if(ExtractDestination(best->pdmnState->scriptPayout, dest)) {
        LogPrintf("GetMNPayee::%s\n",
                  CBitcoinAddress(dest).ToString());
    }
    LogPrintf("GetMNPayee:: end list at %d -------------------------------------------------------- best %s\n", nHeight, CBitcoinAddress(dest).ToString());
    return best;
}
...
bool CDeterministicMNManager::ProcessBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& _state, bool fJustCheck)
{
...

    {
        ...
        if ((nHeight % SNAPSHOT_LIST_PERIOD) == 0 || oldList.GetHeight() == -1) {
            ...
            newList.printList("ProcessBlock snapshot");
        }
    }
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            if (debugLogs) {
                ...
                newList.printList("BuildNewListFromBlock");
            }
